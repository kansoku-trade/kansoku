...
if (newFailures.length > 0) {
  console.error(
    `server test gate FAILED: ${newFailures.length} new failure(s) not in the known-failure allowlist:`,
  );
...
console.log(
  `server test gate OK: ${failed.size} failing test(s), exactly matching the known-failure allowlist.`,
);
