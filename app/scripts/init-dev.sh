...
pnpm_version="$(pnpm -v)"
required_pnpm="$(node -pe "require('$APP_DIR/package.json').packageManager" 2>/dev/null | sed -E 's/^pnpm@//')"
required_major="${required_pnpm%%.*}"
pnpm_major="${pnpm_version%%.*}"
if [ -n "$required_pnpm" ] && [ "$pnpm_major" != "$required_major" ]; then
  echo "警告：当前 pnpm 版本 $pnpm_version，与 app/package.json 声明的 pnpm@$required_pnpm 主版本不一致，可能遇到诡异问题" >&2
else
...
if "$APP_DIR/scripts/fetch-pro.sh"; then
  echo "pro 仓库同步完成"
else
...
if (cd "$APP_DIR" && pnpm -r typecheck); then
  echo "typecheck 通过"
else
  echo "警告：typecheck 未通过，先看 app/README.md 排查，不影响继续开发" >&2
fi
...
下一步：
  cd app && pnpm dev        # 起本地开发环境，http://localhost:5199
  cd app && pnpm dev:desktop  # 桌面壳（Electron），不需要单独的 server 进程

.env 说明：
  - 仓库根目录的 .env 给 Python 技能用（FRED_API_KEY、SEC_USER_AGENT 等），见根 CLAUDE.md
  - app 内 AI 模型/密钥走 /settings 页面存 SQLite；.env 里的 AI_*_MODEL / *_API_KEY 只在首次启动时一次性导入，之后可删
