  ...
  return (
    <Card>
      ...
      <div className="settings-cred-row">
        ...
        <Badge tone={state?.enabled ? 'up' : undefined}>
          {state?.enabled ? '运行中' : '已停用'}
        </Badge>
        <span className="settings-cred-actions">
          <Button
            accent={!state?.enabled}
            disabled={busy || !state}
            onClick={toggle}
          >
            {state?.enabled ? '停用' : '启用'}
      ...
      {error && (
        <div className="settings-test-result settings-test-result--fail">
          {error}
        </div>
      )}

      <div className="settings-footer-note">
        启用后本机会监听 127.0.0.1，供 Claude Code 图表 skill 等命令行工具用
        token 访问；token 等同完整 API
        权限，不要发给外部或暴露到公网。停用或退出应用会关闭该端口。
      </div>
    </Card>
  )
}
