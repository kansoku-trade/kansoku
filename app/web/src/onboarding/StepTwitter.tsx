...
export function StepTwitter({ onComplete }: { onComplete: () => Promise<void> }) {
  const { data, loading, reload } = useQuery<OpencliStatus>("onboarding.opencli", fetchOpencliStatus);
    ...
    <Card className="onboarding-card">
      ...
      {data.state === "not_installed" ? (
        <div className="onboarding-install">
          <pre className="onboarding-cli-command"><code>{OPENCLI_INSTALL_COMMAND}</code></pre>
          <div className="settings-cred-actions">
            <Button onClick={() => void navigator.clipboard.writeText(OPENCLI_INSTALL_COMMAND)}>复制命令</Button>
            <Button onClick={() => window.open(OPENCLI_GITHUB_URL, "_blank", "noopener,noreferrer")}>GitHub 链接</Button>
          </div>
        </div>
      ) : null}
      ...
      {data.state === "extension_missing" ? (
        <div className="onboarding-install">
          ...
          <div className="settings-cred-actions">
            <Button onClick={() => window.open(OPENCLI_RELEASES_URL, "_blank", "noopener,noreferrer")}>下载扩展</Button>
          </div>
        </div>
      ) : null}
