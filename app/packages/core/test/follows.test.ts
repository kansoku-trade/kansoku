...
const { listFollowedSymbols, setSymbolFollowing, symbolFollowState } = await import("../src/ai/follows.js");
...
describe("persistent symbol following", () => {
  it("normalizes and persists a followed symbol", () => {
    const started = setSymbolFollowing("mu", true, undefined, () => new Date("2026-07-14T10:00:00.000Z"));
    expect(started).toEqual({ symbol: "MU.US", following: true, startedAt: "2026-07-14T10:00:00.000Z" });
  ...
  it("keeps the original start time when continue-following is selected twice", () => {
    ...
    const second = setSymbolFollowing("nvda", true, undefined, () => new Date("2026-07-14T11:00:00.000Z"));
  ...
  it("removes a symbol when following is stopped", () => {
    ...
    expect(setSymbolFollowing("amd", false)).toEqual({ symbol: "AMD.US", following: false, startedAt: null });
