...
describe("loadSkillIndex", () => {
  it("parses name and single-line description from frontmatter", () => {
    ...
    expect(index).toEqual([{ name: "alpha", description: "does alpha things", dir: join(root, "alpha") }]);
  });
  ...
  it("joins folded > multi-line description values with spaces", () => {
    ...
    const index = loadSkillIndex([root]);
    expect(index).toEqual([{ name: "beta", description: "first line of the description", dir: join(root, "beta") }]);
  });
  ...
  it("joins folded | multi-line description values with spaces", () => {
    ...
    const index = loadSkillIndex([root]);
    expect(index).toEqual([{ name: "gamma", description: "piped first piped second", dir: join(root, "gamma") }]);
  });
