...
describe("attachAiUsageLogger", () => {
  it("logs each billable message and a run total", () => {
    ...
    attachAiUsageLogger(agent, { layer: "analyst", symbol: "MU.US", model: { provider: "anthropic", id: "x" } });
    ...
    expect(lines.some((l) => l.includes("[ai-usage]") && l.includes("call=1") && l.includes("tokens=110"))).toBe(true);
    expect(lines.some((l) => l.includes("total calls=1") && l.includes("spend=$0.0100"))).toBe(true);
  });
  ...
  it("skips messages without billable usage", () => {
    ...
    expect(log.mock.calls.map((c) => String(c[0])).filter((l) => l.includes("call="))).toHaveLength(0);
  });

  it("does nothing when the agent has no subscribe", () => {
    expect(() => attachAiUsageLogger({}, { layer: "analyst", symbol: "MU.US", model: {} })).not.toThrow();
  });
});
