...
describe("draw_annotations", () => {
  it("exposes polyline as an allowed kind and arrow as a boolean style option in the schema", () => {
    ...
    const kindValues = schema.properties.annotations.items.properties.kind.anyOf.map((k) => k.const);
    expect(kindValues).toContain("polyline");
    expect(schema.properties.annotations.items.properties.style.properties.arrow.type).toBe("boolean");
  });
