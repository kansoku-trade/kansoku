...
3. **一个图表 web 应用** —— `app/`（pnpm workspace：`server/` 是 Tsuki（Hono + NestJS 风格模块/DI）+ TS 写的 kernel，生产态由 `main.node.ts` 单进程托管，开发态是两个进程——server 跑 kernel、web 起 Vite dev server 代理过去，都不需要单独 build 步骤；`web/` 是 Vite + React）在本地 `http://localhost:5199` 渲染全部图表。server 自己调 longbridge CLI 拉数据，指标全部用 TS 算；建图走 `POST /api/charts`（见 `.claude/skills/chart/SKILL.md`）。实时层：单条 WS（`/api/ws`）推送实时报价（watchlist ∪ 持仓，识别盘前/盘后/隔夜）和页面打开期间每 60 秒的图表重算——落盘的图表 JSON 始终冻结在研判那一刻。启动用 `cd app && pnpm start`；测试用 `cd app && pnpm test`。
