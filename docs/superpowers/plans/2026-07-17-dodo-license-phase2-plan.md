...
前置:第一阶段插槽架构已合入 main(pro-slot);`apps/pro` 私有仓库 `Innei/kansoku-pro` 已建。
...
- `apps/pro/src/license/dodoClient.ts`:三端点薄封装,fetch 超时(10s)与网络错误归一化;base URL 按 env 切换。
- `apps/pro/src/license/licenseStore.ts`:加密读写 {key, instanceId, deviceName, lastValidatedAt}。
- `apps/pro/src/license/licenseState.ts`:状态机(全局约束里的五态)+ `getLicenseSnapshot()`(给 capabilities 用)+ `activate(key)` / `deactivate()` / `revalidate()` 动作;deviceName 默认 `os.hostname()`。
...
- 验收:pro 包 typecheck + 测试绿(在 apps/pro 仓库提交)。
...
- `scripts/init-dev.sh`(用户新增需求):一条命令初始化开发环境。检查 node/pnpm → 尝试把 pro 仓库 clone 进 `apps/pro`(默认 `git@github.com:Innei/kansoku-pro.git`,`KANSOKU_PRO_REPO_URL` 可覆盖;有 `apps/pro/.git` 则 pull;无权限则单行提示"进入免费模式"并继续,不中断)→ `pnpm install` → typecheck 冒烟 → 打印下一步提示(`pnpm dev`、`.env` 说明)。幂等,可重复跑;README 的上手章节改为指向它。
...
- 文档:spec 第 2–4 节按 as-built 校订(端点、状态机五态、403 语义);apps/README.md 订阅小节;`apps/pro` README 补 license 模块说明(私有仓库内)。
