...
**Goal:** 把误伤的免费功能从订阅门后放出来（Phase A），再把免费 AI 代码从私有仓 `apps/pro` 搬回开源 core（Phase B）。

**Architecture:** 两阶段各自可发版。Phase A 只动门：pro 仓摘 `LicensedGuard`/`requireLicensed`，core 仓 web 按 licensed 维度解锁并给付费入口上锁。Phase B 按 spec 的搬迁映射把基座与免费功能层移入 `packages/core`，research 模块拆成 core browse + pro AI，`pro-api` 契约收窄。
...
- 两个 git 仓：`trade-skills`（公开，workspace 根即仓库根）与 `apps/pro`（私有 `Innei/kansoku-pro`）。各自独立提交，禁止跨仓 commit。
- 付费三项不动门：`deepDive.ts` 的 `requireLicensed()`、`scheduler.ts` 的 `isLicensed` 检查、license 模块整套保持原样。
- 测试命令：pro 仓 `cd apps/pro && pnpm test`；公开仓 `pnpm test`。只跑受影响的包，勿全量 lint。
...
- [ ] **Step 3:** Run: `cd apps/pro && pnpm test`，Expected: PASS。
- [ ] **Step 4:** Commit（pro 仓）: `git -C apps/pro commit -am "feat: ungate assistant/chat/lobehub from license (free with own key)"`
...
- [ ] **Step 4:** Run: `cd apps/pro && pnpm test`，Expected: PASS。
...
- Modify: `apps/web/src/pages/settings/SettingsPage.tsx`（175 行 `aiUnlocked`、229 行 `!licensed` 分支）
...
- [ ] **Step 6:** Run: `pnpm --filter @kansoku/web test`，Expected: PASS。
...
- Modify: `apps/web/src/LicenseModal.tsx`（文案：只宣传自动跟踪 / deep-dive / 研究库 AI）
...
**Phase A 完成门：** 两仓测试全绿 + `pnpm dev` 手动验证（未订阅：ChatDock 可聊、复评可跑、研究库可浏览；AI 跟进/deep-dive/AI 刷新上锁）。可独立发版。
...
- [ ] **Step 2（移动文件内 import，两条确定性 sed）:** 移入 core 的文件：`../../../../../packages/core/src/` → `../`（core 内相对）；`../../../../packages/shared/` → `../../../shared/`（4up→3up）。intra-ai 的 `./x.js`、`./messages/x.js`、`./lobehub/x.js` 不变。
...
- [ ] **Step 6（双门）:** Run `pnpm --filter @kansoku/core typecheck && pnpm --filter @kansoku/core test` **且** `cd apps/pro && pnpm test`。Expected: 两侧全 PASS（pro 重新完整编译）。
...
> - **启动接线（新子任务，纯开源构建能跑免费 AI 的关键）**：`initAiSettings` 目前只被 pro 的 `initRuntime` 调用。改为 `apps/server/src/runtimeInit.ts` 在 `loadPro` **之前**无条件调用 `initAiSettings(getDb(), { secretBox: opts?.secretBox })`（pro 缺席时免费 AI 仍初始化）。
...
- Modify: `apps/server/src/modules/symbols/symbols.controller.ts`（reassess 两处删 `requirePro()`；deep-dive 两处保留）
- Modify: `apps/server/src/modules/overview/overview.controller.ts`（/usage 删 `requirePro()`）
- Create: desktop 侧免费 IPC 服务（AssistantIpc/ChatIpc/LobeHubIpc 移入公开仓 IPC 注册处，参照 `apps/desktop` 现有 ipc 服务模式）
...
- [ ] **Step 1:** 服务文件搬入 core `modules/`，controller/module 搬入 `apps/server/src/modules/`，import 全部改 core 路径；`aiSettings.service` 已无 license 引用（A3 摘净）。
...
- [ ] **Step 4:** 运行 desktop contract-parity 测试：`pnpm --filter @kansoku/desktop test`。Expected: PASS（allowlist 与 contract 对齐）。
- [ ] **Step 5:** Run: `pnpm test`；`cd apps/pro && pnpm test`。Expected: PASS。
...
- [ ] **Step 2:** contract research group 拆两段；web 客户端 import 对应更新（调用方签名不变则零改动，核对 `apps/web/src/pages/research/researchModel.ts`）。
- [ ] **Step 3:** 补测试：pro 缺席时 `GET /api/research`、`GET /api/research/document` 返回 200；AI 子路由 404。
- [ ] **Step 4:** Run: `pnpm test && cd apps/pro && pnpm test`。Expected: PASS。
...
- [ ] **Step 3:** Run: `pnpm test && cd apps/pro && pnpm test`。Expected: PASS。
...
- [ ] **Step 2:** pro 打包构建验证：`cd apps/pro && pnpm build`（tsdown），Expected: 成功（打出的 dist 供打包桌面宿主用）。
...
- Modify: `apps/web/src/pages/settings/SettingsPage.tsx`（AI 卡片无条件）
- Delete: `apps/web/src/pages/cockpit/chat/LockedChatBar.tsx`、`LicenseGateEmptyState` 相关（若已无引用）
...
- [ ] **Step 3:** Run: `pnpm --filter @kansoku/web test`。Expected: PASS。
- [ ] **Step 4:** Commit: `"feat(web): free AI surfaces render unconditionally"`

### Task B7: 全量验证 + CI

- [ ] **Step 1:** `pnpm test`（全 workspace）与 `cd apps/pro && pnpm test`。Expected: 全绿。
- [ ] **Step 2:** 无 pro 冒烟：临时移开 `apps/pro`（`mv apps/pro /tmp/pro-slot`），`pnpm --filter @kansoku/web build && pnpm typecheck`，`pnpm dev` 验证：研究库可浏览、ChatDock/复评在配 key 后可用、付费 UI 隐藏、`GET /api/capabilities` 报 `{pro:false}`。验完移回。
- [ ] **Step 3:** 触发 CI 的 pro-present build check（fetch-pro + typecheck + build + pro tests）。Expected: 绿。
- [ ] **Step 4:** 更新 `apps/pro/README.md` 与根 `CLAUDE.md` 的 open-core 描述段（免费/付费新边界，一句话即可）。
