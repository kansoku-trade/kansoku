...
Run: `pnpm --filter @trade/desktop add electron-window-state@5.0.3`

Expected: `apps/desktop/package.json` 和 `pnpm-lock.yaml` 只增加该库及其传递依赖。

- [ ] **Step 2: 在创建主窗口前读取状态**

在 `apps/desktop/src/main.ts` 导入 `electron-window-state`，并在 `createWindow()` 开头初始化：
...
Run: `pnpm exec prettier --check desktop/src/main.ts desktop/package.json ../pnpm-lock.yaml`

Expected: 所有目标文件格式正确。

Run: `pnpm --filter @trade/desktop typecheck`

Expected: TypeScript 类型检查通过。

- [ ] **Step 2: 运行桌面包测试**

Run: `pnpm --filter @trade/desktop test`

Expected: 桌面包现有测试全部通过。

- [ ] **Step 3: 构建主进程**

Run: `pnpm --filter @trade/desktop build`
