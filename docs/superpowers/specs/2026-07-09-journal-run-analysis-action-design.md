...
- server：`POST /api/symbols/:sym/reassess`（`apps/server/src/routes/symbols.ts`），
...
### 1. `apps/web/src/pages/cockpit/useReassessSymbol.ts`
...
### 2. `apps/web/src/pages/cockpit/JournalSection.tsx`
...
### 3. `apps/web/src/pages/SymbolCockpit.tsx` / `apps/web/src/pages/cockpit/ReviewTab.tsx`
...
1. `pnpm lint`（仅改动文件）。
2. `pnpm start`，进入某 symbol 页复盘区「日志」标签：
