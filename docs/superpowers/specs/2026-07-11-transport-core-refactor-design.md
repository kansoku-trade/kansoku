...
├── server/                @trade/server —— Tsuki 薄 controller + HTTP/WS host + main.node.ts
├── desktop/               @trade/desktop —— Electron 壳 + src/ipc/（每模块一个 IpcService 薄适配）
└── web/                   @trade/web + src/client/（HTTP 实现 + IPC 实现，按环境二选一）
```
