...
实现上扩展现有 `resolveDataRoot`（`apps/desktop/src/boot/paths.ts`），启动时在 `boot/env.ts` 里先读偏好再解析，最后 `process.env.TRADE_PROJECT_ROOT = dataRoot` 并 `scaffoldDataRoot`。
...
| 职责                            | 建议位置                                                                   |
| ------------------------------- | -------------------------------------------------------------------------- |
| 偏好读写                        | `apps/desktop/src/dataRoot/store.ts`                                       |
| 选目录 / 校验 / 写偏好 / 问重启 | `apps/desktop/src/dataRoot/flow.ts`（参考 `dataImport/flow.ts`）           |
| 解析扩展                        | `apps/desktop/src/boot/paths.ts`                                           |
| 启动接入                        | `apps/desktop/src/boot/env.ts`                                             |
| 菜单                            | `apps/desktop/src/menu/sections/appSection.ts`                             |
| IPC + preload                   | `dataRoot.get` / `pick` / `reset`（命名可微调，壳能力不进 core）           |
| 设置 UI                         | `apps/web` 设置页数据目录区块（仅桌面有意义；server 模式可隐藏或只读展示） |
...
- 更新 `apps/desktop/README.md`：增加「数据目录」说明（默认位置、如何绑仓库、与导入的区别、并发警告）
