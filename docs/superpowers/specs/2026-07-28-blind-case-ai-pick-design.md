...
**为何默认值定得低**：按 §8 实测的 $0.0215/局，$5 约合 **230 局/月**，也就是约 15 次满补货——个人用量下够，但远不是本文初稿声称的「几千局」。它主要不是用来限制正常使用的，是用来兜异常的 —— prompt 写错导致重试风暴、误配成贵十倍的模型、循环调用 bug。这些时候需要的正是一个低得离谱的天花板。

真按每天练几局的节奏用满 230 局，说明确实在高频训练，那时候把额度调高是有依据的决定，而不是拍脑袋。
...
输入走 P0 的编码器。P0 §1 实测字节数：一个 5m 案例从 109,351 压到 33,723 字节，省 69%。

**实测 token（2026-07-28，Electron 实跑 3 次）**：
...
| 单局输出 | 384 token |
| 单局花费 | **$0.0215** |
| 自动补货一次（15 局） | 约 29 万 token / 约 $0.32 |

> **早期估算错在哪。** 本文最初按 4 字符/token 推出「一局约 9k」，实测是它的两倍余。那个系数是英文散文的经验值，而这里的负载几乎全是密集小数——`100.67` 这样的价格会切成好几个 token，密度远高于散文。
>
> **字节层面的 69% 压缩不受影响**：JSON 编码的 token 数会同比例更高。错的只是绝对值的换算，不是压缩结论。
