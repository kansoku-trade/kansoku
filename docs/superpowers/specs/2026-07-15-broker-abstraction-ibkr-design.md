...
- `packages/core/src/services/marketdata/` 已有干净的插件缝隙：`MarketDataProvider` 接口（取数）、`QuoteStream` 接口（实时推送）、`registry.ts`（按市场 + env 变量选 provider），但表里只有长桥一家。
- 长桥在抽象层之外的泄漏点：
...
5. `grep -r "longbridge" packages/core/src` 零命中（接口、提示词、报错 hint 全部中性化；长桥字样只存在于 `broker-longbridge` 包内）。
