...
**块的上下边界钳在面板矩形里**（`0 ~ paneSize().height`）。价格被缩放到面板之外时 `priceToCoordinate` 照样给得出坐标，不钳的话 `fillRect` 会一路铺到画布边缘、`strokeRect` 的那条边和块中央的 R 值都落到画布外面——画出来就是一片没有底边、也没有数字的开口红区，等于宣告了一条图上拒绝命名的风险边界。钳到面板边之后，块的边和 DOM 那一层钉在同一条边上的药丸（见下）停在同一个位置，两层说的是同一件事。两条边钳到同一侧时块高为 0，整块不画。
...
**「全宽」指的是 K 线面板那一块的宽度，不是整个 overlay 的宽度。** 左右边界按测到的面板矩形逐帧算（`usePinnedPriceYs` 顺带把面板尺寸一起量出来），右边到价格轴为止：那条带子是看不见的，越界一点就会把价格轴自己的「拖着缩放」吃掉。左侧画笔工具栏（`.trainer-overlay-rail`）比命中带晚一层，所以给它抬了 `z-index`，否则一条横穿工具栏的线会在按钮上盖一层看不见的 9px 带子。

### 价格缩放到面板之外：撤线撤带，药丸钉边

`priceToCoordinate` 对一个超出价格轴范围的价格仍然会给出一个坐标，照着画就画到价格轴、画笔工具栏或下面那张 MACD 图上去了。**撤掉的只有线和它的命中带**：按真实 y 画会越界，按钳过的 y 画则是在一个不存在的价位上画一条线，两种都不行。

**整行不能跟着一起撤。** 这一行的药丸是好几样控件的唯一容器：

- 入场药丸是三个仓位提交钮（`进场 1/4 1/2 全仓`）和 `撤销这个计划` 唯一的落脚点，底栏 `TrainerEntryLane` 只有方向钮和市价快捷入场，没有带仓位的提交。入场价一旦被自动缩放甩出去，整张画好的、合法的单子就再也发不出去了。
- 持仓改单的 `确认调整` / `撤销` 长在被拖动那条线自己的药丸上。止损拖得太开、松手时落在面板外，这笔待确认的改单就卡在状态里，没有任何地方能确认或撤回。
- 拖动监听挂在 `window` 上、父组件不卸载，所以整行被撤会在手势进行到一半时把药丸拆掉——光标底下的价格读数当场消失，人还在拖。

所以行保留，药丸钉在面板边上（离边 `PILL_EDGE_INSET_PX = 14`，正好是半个药丸的高度，不越到 MACD 那张图上）。**钉住的药丸要看得出是钉住的，不能读成一个真价格**：边框转虚线、透明度压一档，价格前面加一个方向小三角（`▴` 在上方 / `▾` 在下方），`aria-label` 补一句「超出图表范围」，`title` 说明可以拖它把线拉回来。

药丸自己就是拖动面，`onPointerDown` 一直挂着（`TrainerOrderLevels.tsx` 把 `onGrab` 同时给了命中带和药丸），所以按住钉住的药丸往图里拖，就能把这条线拉回价格轴覆盖的范围——TD-EXIT-01 需要的那条「止损跑出视野后还能上移锁利」的回收路径靠这个成立。

入场那一行本来就不需要撤：`kind === 'entry'` 时 `onGrab` 是 `undefined`，它压根没有命中带可以越界。
...
- 区块的越界钳制：止损被缩放到面板下方时红区底边停在面板边上、块中央的 R 值仍落在面板内；入场被甩到面板上方时顶边停在 0；两条边同在一侧时整块不画
- 标签收起 / 展开两态的渲染：收起时只有价格、展开时仓位按钮可点、`dragging` 期间即使鼠标移开也保持展开
- 命中带能触发拖动，且与 pill 走同一条价格回调；命中带的左右边界等于 K 线面板而不是整个 overlay
- 价格被缩放到面板之外时：线和命中带都不画，药丸仍在并钉在面板边上、带方向小三角和「超出图表范围」的 `aria-label`；canvas 区块和这一行说的是同一个止损价；入场被甩出去时三个仓位提交钮和 `撤销这个计划` 仍可点且能真的发出去；持仓改单被甩出去时 `确认调整` / `撤销` 仍可点；按住钉住的药丸能把线拖回面板内
- 拖动纪律测试一律从命中带上按下去，不从画布上按——命中带是真实鼠标唯一能落到的地方，走别的路径测出来的结论不代表线上行为
- 一次把线拖出面板的手势里，药丸的价格读数全程跟手，不会在中途消失
