...
图表追问 chat（`packages/core/src/ai/chat.ts`）的全部工具在构建时绑死在当前图表的标的上（`read_data_pack` / `fetch_kline` / `fetch_news` / `read_drawings` / `draw_annotations`，工具参数里没有 symbol 字段），也没有 bash / read_skill / read_file。用户追问一旦外溢到板块或关联标的（SMH、SK Hynix、ASML），agent 只能回答「我查不到」。2026-07-16 的实际对话截图证实了这一点。
...
位置：`packages/core/src/ai/agentTools.ts`（三个单工具 builder 已在此文件）。
...
新文件：`packages/core/src/ai/messages/sharedProviders.ts`。从 `analystMessagesEngine.ts` 迁出并泛化：
