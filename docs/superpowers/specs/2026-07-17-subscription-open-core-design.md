...
Dodo 的 activate / validate / deactivate 是公开端点,客户端直连,不需要自己的 license 服务器。全部实现在 `apps/pro/src/license/`(闭源),公开侧只认识 `capabilities` 里的 `licensed` 布尔值和 license 路由的类型。
