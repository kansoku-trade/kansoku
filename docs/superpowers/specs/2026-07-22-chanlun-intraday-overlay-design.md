...
新增到 `packages/shared/types.ts`（跟 `IntradayTfData` / `SeriesMarker` 定义同一文件；shared 包是扁平布局，无 `src/` 子目录）：
...
| 文件 | 改动 |
|---|---|
| `packages/shared/types.ts` | 新增 `Fenxing / Bi / Xianduan / Zhongshu / BuySellPoint / ChanStructure / PriceRectangle`；`SeriesMarker.group` union 扩展 10 值 |
...
**测试框架 · vitest**（`pnpm --filter @kansoku/core test` 或工作区根 `pnpm test`）。**单元测试**用 golden fixtures（JSON 固化的 K 线输入 + 期望输出），便于回归。ESM `.js` 扩展名 import 惯例沿用现有 `packages/core/src/analysis/**`。每层覆盖典型正面 + 至少一个反面（不成立场景 + 边界）：
