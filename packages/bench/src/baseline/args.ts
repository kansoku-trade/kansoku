...
function parseModes(value: string): MockMode[] {
  ...
  for (const mode of modes) {
    if (mode !== 'blind' && mode !== 'live')
      throw new Error(`invalid --mode: ${mode} (expected blind|live)`);
  }
  return modes as MockMode[];
}

function parseBank(value: string): 'swing' | 'intraday' {
  if (value !== 'swing' && value !== 'intraday')
    throw new Error(`invalid --bank: ${value} (expected swing|intraday)`);
  return value;
}
