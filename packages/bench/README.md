...
## 这套基准能分辨多大的差别（读结果之前先读这一节）

单个 episode 的净 R 噪音极大。2026-07-21 用 `gpt-5.4-mini` 在 `v2-blind-pilot` 上跑了 12 道题 × 3 遍（36 局，引擎修复后），实测：

| 量 | 值 |
| --- | --- |
| 同题内标准差（单局噪音） | **2.24R** |
| 题间标准差（题目难度差异） | 1.25R |
| 12 题 × 3 遍下总体均值的 95% 区间 | **±0.71R** |
| 配对设计（同题两臂）能检测的最小差值 | **±1.03R** |

同一个模型、同一道题、什么都不改重跑三遍，净 R 极差的中位数是 4.38R，最大 8.58R。个别题目三遍分别是 `+3.67 / −0.37 / −6.21`。

由此得到一条硬规则：

> **单局或单题层面小于约 4.5R 的差值，没有 3 遍以上重复就不能当结论；跨模型或跨配置的整体差值小于约 1R，在 12 题规模下根本不可检测。**

要检测更小的效应，需要的是**更多题目**而不是更多重复——真正的样本量是独立的行情路径数：

| 想检测的效应 | 配对设计所需题目（每题 3 遍） | episode 总数 |
| --- | ---: | ---: |
| 0.5R / 题 | 约 52 | 312 |
| 0.25R / 题 | 约 206 | 1236 |

这一节存在的原因是：2026-07-21 当天曾据单次运行得出「止损小于 2 ATR 必被扫」「限制出手次数能把总收益由负转正」「第 4 笔之后交易质量塌方」三条结论，扩大样本后全部不成立。**先算噪音带，再解读差值。**
