...
function sourceQuestion(cutoff = "2024-03-27", horizonSessions = 4) {
  const hourDates = businessDates(dateOffset(cutoff, -60), dateOffset(cutoff, horizonSessions * 2 + 20));
  ...
  const hourBars = [...initial, ...future].flatMap((date, dayIndex) =>
    Array.from({ length: 7 }, (_, hourIndex) =>
      bar(marketTime(date, `${String(9 + hourIndex).padStart(2, "0")}:30:00`), dayIndex * 7 + hourIndex)),
  );
...
describe("blind episode anonymization", () => {
  it("removes identity and event fields while preserving market geometry", () => {
    ...
    const blindReturn = Number(question.replay.bars[0].close) / Number(question.fixtures.quote.last);
  ...
  it("preserves New York market hours across a shifted DST boundary", () => {
    ...
    const failedChecks = auditEpisodeQuestion(question).checks
