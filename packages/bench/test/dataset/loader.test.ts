...
const malformedExtraKeyFile = fileURLToPath(
  new URL('../fixtures/malformed-extra-key.json', import.meta.url),
);
...
describe('dataset loader', () => {
  ...
  it('loadQuestionForRunner strips replay as an own property', async () => {
    const runnerQuestion = await loadQuestionForRunner(
      fixturesRoot,
      'v1',
      'swing',
      'swing-TEST-01',
  ...
  it('rejects a malformed fixture missing cutoff, naming the file and failing path', async () => {
    await expect(loadQuestionFile(malformedMissingCutoffFile)).rejects.toThrow(
      DatasetValidationError,
    );
    await expect(loadQuestionFile(malformedMissingCutoffFile)).rejects.toThrow(
      /malformed-missing-cutoff\.json/,
    );
  ...
  it('rejects a fixture with an unknown top-level key', async () => {
    ...
    await expect(loadQuestionFile(malformedExtraKeyFile)).rejects.toThrow(
      /malformed-extra-key\.json/,
    );
  });
});
