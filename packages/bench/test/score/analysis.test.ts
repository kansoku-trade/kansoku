...
describe('agreementMatrixOf', () => {
  ...
  it('combines reps by majority direction and counts a tie as disagreement', () => {
    ...
    for (let i = 0; i < 5; i++) {
      cells.push(
        mkCell({ model: 'a', questionId: `q${i}`, mode: 'blind', rep: 0, direction: 'long' }),
      );
      cells.push(
        mkCell({ model: 'b', questionId: `q${i}`, mode: 'blind', rep: 0, direction: 'long' }),
      );
    }
    cells.push(
      mkCell({ model: 'a', questionId: 'q-tie', mode: 'blind', rep: 0, direction: 'long' }),
    );
    cells.push(
      mkCell({ model: 'a', questionId: 'q-tie', mode: 'blind', rep: 1, direction: 'short' }),
    );
    cells.push(
      mkCell({ model: 'b', questionId: 'q-tie', mode: 'blind', rep: 0, direction: 'long' }),
    );
