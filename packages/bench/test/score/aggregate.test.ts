...
describe('judgmentSummary arithmetic', () => {
  ...
  it('timeout_flat with score <= 0 counts loss-side', () => {
    const s = judgmentSummary(
      [mkCell({ outcome: 'win', score: 2 }), mkCell({ outcome: 'timeout_flat', score: -0.5 })],
      0,
    );
...
describe('aggregate efficiency min-max', () => {
  function withCost(model: string, cost: number, dur: number) {
    return mkCell({
      ...
      outcome: 'win',
      score: 1,
      metrics: { durationMs: dur, costUsd: cost, toolCalls: 0 },
    });
  }
  ...
  it('three models normalize linearly', () => {
    const models = aggregate(
      [withCost('a', 10, 10), withCost('b', 20, 20), withCost('c', 30, 30)],
      WEIGHTS,
    );
  ...
  it('scores baseline/gold on judgment alone and gives them null efficiency', () => {
    const cells = [
      mkCell({
        ...
        outcome: 'win',
        score: 1,
        metrics: { durationMs: 100, costUsd: 0.02, toolCalls: 0 },
      }),
      mkCell({
  ...
  it('keeps a zero-cost baseline out of the min-max efficiency pool', () => {
    const cells = [
      mkCell({
        ...
        outcome: 'win',
        score: 1,
        metrics: { durationMs: 100, costUsd: 0.01, toolCalls: 0 },
      }),
      mkCell({
        model: 'b',
        outcome: 'win',
        score: 1,
        metrics: { durationMs: 200, costUsd: 0.02, toolCalls: 0 },
      }),
      mkCell({
        model: 'baseline/buy-hold',
        outcome: 'win',
...
describe('abstainRate', () => {
  ...
  it('is 0 when a model never abstains', () => {
    const s = judgmentSummary(
      [mkCell({ outcome: 'win', score: 1 }), mkCell({ outcome: 'loss', score: -1 })],
      0,
    );
...
describe('avgWinnerR', () => {
  ...
  it('equals the single winning score when there is exactly one win', () => {
    const [m] = aggregate(
      [mkCell({ outcome: 'win', score: 2.5 }), mkCell({ outcome: 'loss', score: -1 })],
      WEIGHTS,
    );
