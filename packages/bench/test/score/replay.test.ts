...
describe('replayDirectional sanity gate', () => {
  ...
  it('long stop on wrong side', () => {
    expect(
      replayDirectional({ direction: 'long', entry: 100, stop: 105, target: 120, bars }).outcome,
    ).toBe('format_violation');
  });
  it('short stop on wrong side', () => {
    expect(
      replayDirectional({ direction: 'short', entry: 100, stop: 95, target: 80, bars }).outcome,
    ).toBe('format_violation');
  });
  it('long target on wrong side', () => {
    expect(
      replayDirectional({ direction: 'long', entry: 100, stop: 90, target: 95, bars }).outcome,
    ).toBe('format_violation');
  });
  it('non-finite price', () => {
    expect(
      replayDirectional({ direction: 'long', entry: Number.NaN, stop: 90, target: 120, bars })
        .outcome,
    ).toBe('format_violation');
  });
  it('R <= 0 via equal entry/target', () => {
    expect(
      replayDirectional({ direction: 'long', entry: 100, stop: 90, target: 100, bars }).outcome,
    ).toBe('format_violation');
  });
});
