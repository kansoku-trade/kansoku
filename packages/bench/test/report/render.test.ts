...
function buildScoresFrom(runId: string, datasetVersion: string, cells: CellVerdict[]): Scores {
  ...
  return {
    ...
    cells,
    models,
    analysis: computeAnalysis(cells),
  };
}
...
describe('renderReport', () => {
  ...
  it('renders 模型同质化矩阵 as a GFM table', () => {
    ...
    const headerIdx2 = lines.findIndex(
      (l) => l.startsWith('| model |') && l.includes('openai/gpt-5'),
    );
  ...
  it('renders 单题钻取 with 入场/止损/目标 columns for a directional cell', () => {
    ...
    expect(lines[headerIdx]).toBe(
      '| model | mode | rep | 方向 | 入场 | 止损 | 目标 | 结果 | 得分 | trace |',
    );
  ...
  it('shows format_violation outcome for a failed cell with — prices', () => {
    ...
    const row = lines
      .slice(sectionStart)
      .find(
        (l) => l.startsWith('|') && l.includes('openai/gpt-5') && l.includes('format_violation'),
      );
    expect(row).toBeDefined();
    const cells = row!.split('|').map((c) => c.trim());
    expect(cells[4]).toBe('—');
  ...
  it('summary ranks by total and flags models beating buy-hold', () => {
    expect(summary.ranking[0].total).toBeGreaterThanOrEqual(summary.ranking.at(-1)!.total);
  ...
  it("uses the cell's stored traceRef verbatim for the drilldown link", () => {
    const cells = [
      mkCell({
        ...
        score: 1,
        r: 1,
        traceRef: 'custom/path.trace.jsonl',
      }),
    ];
...
describe('renderReport escaping and determinism', () => {
  it('escapes raw pipes in model names', () => {
    const cells = [
      mkCell({ model: 'weird|model', questionId: 'q1', outcome: 'win', score: 1, r: 1 }),
    ];
...
describe('renderReport single-mode run', () => {
  ...
  it('renders — for repeat when config is missing', () => {
    const cells = [
      mkCell({ model: 'solo/model', questionId: 'q1', outcome: 'win', score: 1, r: 1 }),
    ];
...
describe('renderReport difficulty tiers and agreement matrix rendering', () => {
  ...
  it('escapes pipes in question ids inside the difficulty lists', () => {
    const cells = [
      mkCell({ model: 'solo/model', questionId: 'weird|q', outcome: 'win', score: 1, r: 1 }),
    ];
