...
describe('enumerateArchiveGrid', () => {
  ...
  it('ends at the cutoff instant and starts 48h earlier, exclusive of the start boundary', () => {
    const stamps = enumerateArchiveGrid(CUTOFF);
    expect(stamps.at(-1)).toBe(formatArchiveStamp(Date.parse(CUTOFF)));
    expect(stamps[0]).toBe(
      formatArchiveStamp(Date.parse(CUTOFF) - 48 * 60 * 60 * 1000 + 15 * 60 * 1000),
    );
  });
...
describe('archiveFileUrl', () => {
  it('builds the direct gkg zip download URL for a grid stamp', () => {
    expect(archiveFileUrl('20260323133000')).toBe(
      'http://data.gdeltproject.org/gdeltv2/20260323133000.gkg.csv.zip',
    );
  });
});
