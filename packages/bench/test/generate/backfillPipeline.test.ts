...
describe('runBackfillNews', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      ...
      JSON.stringify(
        baseQuestion('MU.US', 'swing-MU-2026-03-19-01', '2026-03-19T20:00:00-04:00'),
        null,
        2,
      ),
      'utf8',
    ...
    await writeFile(
      ...
      JSON.stringify(
        baseQuestion('SPY.US', 'swing-SPY-2026-03-19-01', '2026-03-19T20:00:00-04:00'),
        null,
        2,
      ),
      'utf8',
  ...
  it('skips GDELT/EDGAR entirely for ETF questions (null companyQuery/cik)', async () => {
    ...
    expect(etf).toEqual({
      id: 'swing-SPY-2026-03-19-01',
      symbol: 'SPY.US',
      gdeltCount: 0,
      edgarCount: 0,
    });

    const rewritten = await loadQuestionFile(
      join(datasetsRoot, 'v1', 'swing', 'swing-SPY-2026-03-19-01.json'),
    );
  ...
  it('rewrites fixtures.news in place and round-trips through schema validation', async () => {
    ...
    const rewritten = await loadQuestionFile(
      join(datasetsRoot, 'v1', 'swing', 'swing-MU-2026-03-19-01.json'),
    );
    expect(rewritten.fixtures.news).toHaveLength(2);
    expect(rewritten.fixtures.news.every((item) => item.published_at <= rewritten.cutoff)).toBe(
      true,
    );
  });
  ...
  it('does not write files in dry-run mode', async () => {
    ...
    const untouched = await loadQuestionFile(
      join(datasetsRoot, 'v1', 'swing', 'swing-MU-2026-03-19-01.json'),
    );
  ...
  it('trips the GDELT circuit breaker after consecutive failures and skips remaining GDELT calls', async () => {
    await writeFile(
      ...
      JSON.stringify(
        baseQuestion('AMD.US', 'swing-AMD-2026-03-19-01', '2026-03-19T20:00:00-04:00'),
        null,
        2,
      ),
      'utf8',
    ...
    await writeFile(
      ...
      JSON.stringify(
        baseQuestion('NVDA.US', 'swing-NVDA-2026-03-19-01', '2026-03-19T20:00:00-04:00'),
        null,
        2,
      ),
      'utf8',
  ...
  it('news-source archive: fetches org matches from the injected archive fetcher without calling DOC', async () => {
    ...
    const { options } = makeDeps({
      ...
      readArchiveCsv: async () => {
        ...
        return archiveRow(
          '20260319100000',
          'example.com',
          'https://example.com/micron-earnings-preview',
          'micron technology',
        );
      },
    });
    ...
    expect(rewritten.fixtures.news.some((item) => item.source === 'gdelt-arch:example.com')).toBe(
      true,
    );
  });
      ...
      readArchiveCsv: async () => {
        ...
        return [
          ...
          archiveRow(
            '20260319100000',
            'example.com',
            'https://example.com/amd-rally-story',
            'advanced micro devices',
          ),
          archiveRow(
            '20260319100000',
            'example.com',
            'https://example.com/nvidia-launch-story',
            'nvidia corp',
          ),
        ].join('\n');
      },
