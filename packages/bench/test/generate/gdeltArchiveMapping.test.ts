...
describe('parseGkgRow', () => {
  it('parses a real GKG 2.1 row against verified column indices', async () => {
    ...
    expect(row?.url).toBe(
      'https://example.com/investing/2026/03/23/micron-technology-beats-earnings-estimates/',
    );
...
describe('isEnglishRow', () => {
  ...
  it('treats a non-empty TranslationInfo as non-English', () => {
    const cols = Array.from({ length: 26 }).fill('');
...
const PG_TERMS: ArchiveTerms = {
  strongTerms: ['procter gamble', 'procter and gamble'],
  weakTerm: 'procter',
};
...
describe('rowMatchesCompany', () => {
  ...
  it('falls back to matching the URL via weak term + finance-context corroborator', () => {
    const row = rowFrom({
      org: 'tim cook',
      url: 'https://example.com/2026/03/23/microsoft-earnings-beat-estimates',
    });
  ...
  it('rejects: harraseeket freeport maine ice bar (weak-only, no corroborator)', () => {
    const row = rowFrom({
      org: 'harraseeket inn',
      url: 'https://example.com/travel/harraseeket-freeport-maine-ice-bar',
    });
  ...
  it('rejects: avatar fire and ash digital release (orgs-tagged procter, no corroborator)', () => {
    const row = rowFrom({
      org: 'procter',
      url: 'https://example.com/movies/avatar-fire-and-ash-digital-release',
    });
  ...
  it('rejects: LRB Andrew Marvell literary piece (weak-only, no corroborator)', () => {
    const row = rowFrom({
      org: 'andrew marvell',
      url: 'https://example.com/books/lrb-andrew-marvell-poetry-essay',
    });
  ...
  it('rejects: hungry caterpillar book piece (weak-only, no corroborator)', () => {
    const row = rowFrom({
      org: 'eric carle',
      url: 'https://example.com/books/the-very-hungry-caterpillar-review',
    });
  ...
  it('accepts: weak micron article with stock in slug via corroborator', () => {
    const row = rowFrom({
      org: 'micron',
      url: 'https://example.com/investing/micron-stock-rallies-today',
    });
    expect(rowMatchesCompany(row, MU_TERMS, 'MU.US')).toBe(true);
  });
  ...
  it('keeps the amdocs-style substring guard for weak single tokens', () => {
    ...
    const row = rowFrom({
      org: 'amdocs ltd',
      url: 'https://example.com/tech/amdocs-quarterly-report',
    });
...
describe('deriveTitleFromUrl', () => {
  it('derives a title from a hyphenated slug', () => {
    ...
    ).toBe('Micron Technology Beats Earnings Estimates');
  });

  it('strips a trailing numeric id from a slug', () => {
    expect(
      deriveTitleFromUrl(
        'https://finance.yahoo.com/markets/stocks/articles/sectors-not-getting-hit-market-123500143.html',
      ),
    ).toBe('Sectors Not Getting Hit Market');
  });
  ...
  it('falls back to an earlier path segment when the last one is a bare numeric id', () => {
    expect(deriveTitleFromUrl('https://example.com/micron-earnings-report/12345')).toBe(
  ...
  it('skips an unknown-extension numeric id (126309170.cms)', () => {
    expect(
      deriveTitleFromUrl('https://timesofindia.example.com/business/126309170.cms'),
    ).toBeNull();
  });
  ...
  it('leaves normal hyphen slugs unaffected', () => {
    expect(
      deriveTitleFromUrl('https://example.com/why-micron-stock-crashed-after-blowout-earnings'),
    ).toBe('Why Micron Stock Crashed After Blowout Earnings');
  });
});
...
describe('mapArchiveMatches', () => {
  ...
  it('dedupes by normalized derived title', () => {
    const items = mapArchiveMatches(
      [
        match({ url: 'https://a.com/micron-beats-estimates' }),
        match({ url: 'https://b.com/micron-beats-estimates' }),
      ],
      CUTOFF,
    );
  ...
  it('sorts by date descending and caps at 10', () => {
    const matches = Array.from({ length: 15 }, (_, i) =>
      match({
        url: `https://a.com/story-alpha-${String.fromCharCode(97 + i)}`,
        date: '20260323133000',
      }),
    );
