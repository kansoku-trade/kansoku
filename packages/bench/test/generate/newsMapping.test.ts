...
describe('normalizeTitle', () => {
  it('ignores case, punctuation, and surrounding whitespace', () => {
    expect(normalizeTitle('  Micron Beats!  Estimates.  ')).toBe(
      normalizeTitle('micron beats estimates'),
    );
  });
});
...
describe('mapEdgarFilings', () => {
  ...
  it('keeps filings inside the 14-day window and drops those outside', () => {
    const items = mapEdgarFilings(
      [
        filing({ filingDate: '2026-03-10' }),
        filing({ filingDate: '2026-02-20', accessionNumber: '0000723125-26-000001' }),
      ],
      CUTOFF,
      '0000723125',
      window.startDate,
...
describe('assertNoLeak', () => {
  it('passes when every item is on or before cutoff', () => {
    expect(() =>
      assertNoLeak(
        [{ id: 'x', title: 't', published_at: '2026-03-19T12:00:00Z', url: 'u' }],
        CUTOFF,
      ),
    ).not.toThrow();
  });
  ...
  it('throws when an item leaks past cutoff', () => {
    expect(() =>
      assertNoLeak(
        [{ id: 'x', title: 't', published_at: '2026-03-21T00:00:00Z', url: 'u' }],
        CUTOFF,
      ),
