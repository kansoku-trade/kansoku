...
ruleTester.run('no-escaping-import', overlayPlugin.rules['no-escaping-import'], {
  invalid: [
    ...
    },
    {
      ...
      filename: '/repo/apps/pro/overlays/packages/build-overlay/poc/edition.pro.ts',
...
describe('overlay-manifest-consistency (fs fixtures)', () => {
  ...
  writeFile(join(fixture.publicRoot, 'packages', 'build-overlay', 'poc', 'e.ts'));
  ...
  ruleTester.run('overlay-manifest-consistency', overlayPlugin.rules['overlay-manifest-consistency'], {
    ...
    valid: [
      ...
      },
      {
        ...
        filename: join(fixture.overlayRoot, 'packages', 'build-overlay', 'poc', 'e.pro.ts'),
        options,
      },
      {
        code: 'export const a = 1;',
