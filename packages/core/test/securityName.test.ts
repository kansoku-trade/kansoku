...
describe("security display name", () => {
  it("prefers the Longbridge Chinese name over an English fallback", async () => {
    ...
    await expect(resolveSecurityName("MRVL.US", "Marvell Technology", provider(getSecurityName))).resolves.toBe(
      "迈威尔科技",
    );
  });
  ...
  it("preserves an explicit Chinese name without another lookup", async () => {
    ...
    await expect(resolveSecurityName("MRVL.US", "迈威尔科技", provider(getSecurityName))).resolves.toBe("迈威尔科技");
