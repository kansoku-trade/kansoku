...
describe("longbridge CLI boundary", () => {
  it("prefers LONGBRIDGE_CLI_PATH over PATH", async () => {
    const cli = fakeCli();
    await expect(locateLongbridgeCli({ env: { LONGBRIDGE_CLI_PATH: cli, PATH: "" } })).resolves.toBe(cli);
  });
  ...
  it("executes without a shell, appends JSON output mode, and parses stdout", async () => {
    ...
    expect(exec).toHaveBeenCalledWith(cli, ["quote", "AAPL.US", "--format", "json"], expect.any(Object));
  });

  it("does not include stdout in an invalid JSON error", async () => {
    const cli = fakeCli();
    const exec = vi.fn().mockResolvedValue({ stdout: "secret-token", stderr: "" });
    const error = await runLongbridgeJson([], { env: { LONGBRIDGE_CLI_PATH: cli, PATH: "" }, exec }).catch((e) => e);
