  ...
  it('normalizes each series independently to first-bar pct', () => {
    const series = [
      ...
      },
      {
        symbol: 'QQQ',
        bars: [
          bar('2026-07-01T13:30:00Z', 50, 51, 49, 50),
          bar('2026-07-02T13:30:00Z', 50, 56, 49, 55),
        ],
      },
    ];
...
describe('buildCockpitPosition', () => {
  ...
  it('computes distances for a full plan', () => {
    const result = buildCockpitPosition(positions, 'MRVL.US', 310, {
      stop: 290,
      target1: 330,
      target2: 350,
    });
...
describe('judgeOutcome', () => {
  ...
  it('short hit_target when low touches target1', () => {
    const bars: RawBar[] = [bar('2026-07-01T13:31:00Z', 100, 101, 90, 91)];
    const result = judgeOutcome('short', anchor, { stop: 110, target1: 92 }, bars);
    expect(result?.status).toBe('hit_target');
  });

  it('short hit_stop when high touches stop', () => {
    const bars: RawBar[] = [bar('2026-07-01T13:31:00Z', 100, 112, 99, 110)];
...
describe('zoneFromPrediction', () => {
  it('reads low/high from range_bound_plan or range_plan alias', () => {
    expect(zoneFromPrediction({ range_bound_plan: { low: 95, high: 105 } })).toEqual({
      low: 95,
      high: 105,
    });
    expect(zoneFromPrediction({ range_plan: { low: 95, high: 105 } })).toEqual({
      low: 95,
      high: 105,
    });
  });
