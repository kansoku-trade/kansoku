...
function encryptedToken(payload: object, machineId: string): Buffer {
  ...
  const key = Buffer.from(
    hkdfSync("sha256", Buffer.from(machineId), Buffer.alloc(0), Buffer.from("longbridge-token-v1"), 32),
  );
...
describe("Longbridge CLI token reader", () => {
  ...
  it("reads the legacy plaintext token format", async () => {
    ...
    getLongbridgeAuthStatus.mockResolvedValue({ token: { status: "valid", path, dc_region: "us" } });
  ...
  it("decrypts the current machine-bound token format", async () => {
    ...
    getLongbridgeAuthStatus.mockResolvedValue({ token: { status: "valid", path, dc_region: "us" } });
    await expect(readLongbridgeToken({ machineId: async () => "machine-id" })).resolves.toMatchObject({
      clientId: "client-id",
      accessToken: "us_fake-access-token",
    });
  });
  ...
  it("never exposes encrypted bytes or tokens when decryption fails", async () => {
    ...
    const error = await readLongbridgeToken({ machineId: async () => "wrong-machine" }).catch((cause) => cause);
