...
describe('WebApiLobeHubCloudGateway', () => {
  ...
  it('completes Device Flow after pending authorization and stores OAuth tokens', async () => {
    ...
    await expect(gateway.pollDeviceLogin()).resolves.toEqual({
      status: 'pending',
      intervalSeconds: 2,
    });
  ...
  it('maps the requested thinking level through thinkingLevelMap before sending reasoning_effort', async () => {
    ...
    for await (const _e of gateway.stream(deepseek, context, { reasoning: 'minimal' } as never))
      void _e;
    for await (const _e of gateway.stream(deepseek, context, { reasoning: 'xhigh' } as never))
      void _e;
    for await (const _e of gateway.stream(model, context, { reasoning: 'minimal' } as never))
      void _e;
  ...
  it('serializes refresh-token rotation across concurrent quota requests', async () => {
    ...
    const fetcher = vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.includes('subscription.getSubscription')) {
        return json({
          result: {
            data: {
              json: { plan: 'pro', usage: { free: { limit: 500_000, boundedSpend: 100_000 } } },
            },
          },
        });
      }
    ...
    expect(first).toMatchObject({
      availableCredits: 400_000,
      availableUsd: 0.4,
      currentMonthUsd: 0.025,
    });
