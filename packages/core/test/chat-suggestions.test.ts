...
const ctx = vi.hoisted(() => {
  ...
  return { dir: `${base}${sep}chat-suggest-test-${Date.now()}-${Math.random().toString(36).slice(2)}` };
});

vi.mock("../src/env.js", () => ({ CHART_DATA_DIR: ctx.dir }));

const { buildChatSuggestions, clearChatSuggestionCache } = await import("../src/ai/chatSuggestions.js");
...
describe("buildChatSuggestions", () => {
  ...
  it("returns the generated questions", async () => {
    const deps = baseDeps({ agentFactory: submitFactory(["凭什么说偏空？", "失效位怎么来的", "和 SMH 比强弱"]) });
