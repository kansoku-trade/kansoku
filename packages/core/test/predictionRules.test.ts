...
describe("validatePrediction", () => {
  ...
  it("passes a neutral call using the range_bound_plan spelling", () => {
    ...
    expect(
      validatePrediction({
        ...rest,
        direction: "neutral",
        range_bound_plan: { low: 97, high: 104, long_tactic: "回踩 97 收稳做多", short_tactic: "反抽 104 受阻做空" },
      }),
    ).toEqual([]);
  });
  ...
  it("rejects string-typed entry/stop instead of silently passing", () => {
    const issues = validatePrediction({
      ...validPrediction,
      entry_plan: { entry: "100" as unknown as number, stop: "97" as unknown as number, target1: 104 },
    });
