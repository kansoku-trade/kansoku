...
describe("outcome cache", () => {
  it("stores resolved outcomes and reads them back", async () => {
    ...
    await saveResolvedOutcome({ chartId: "c1", symbol: "MU.US", direction: "long" }, outcome("hit_target", 4.2), db);
  ...
  it("refuses to store open outcomes", async () => {
    const db = createDb(":memory:");
    await saveResolvedOutcome({ chartId: "c1", symbol: "MU.US", direction: "long" }, outcome("open"), db);
    expect((await getResolvedOutcomes(["c1"], db)).size).toBe(0);
  });

  it("keeps the first resolution on duplicate saves", async () => {
    const db = createDb(":memory:");
    await saveResolvedOutcome({ chartId: "c1", symbol: "MU.US", direction: "long" }, outcome("hit_target", 4), db);
    await saveResolvedOutcome({ chartId: "c1", symbol: "MU.US", direction: "long" }, outcome("hit_stop", -9), db);
