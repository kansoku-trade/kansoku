...
describe('detectCandlePatterns', () => {
  ...
  it('detects hammer only when a prior downtrend exists before the candle', () => {
    ...
    expect(
      detect([smallGreen(100), smallRed(100.5), smallGreen(100.2), smallRed(100.4), hammer]),
    ).toHaveLength(0);

    expect(
      detect([smallRed(104), smallRed(103), smallRed(102), smallRed(101), hammer]).map(
        (p) => p.kind,
      ),
    ).toContain('hammer');
  });
  ...
  it('attaches confirm/invalidate prices to directional patterns and nulls for neutral ones', () => {
    ...
    const neutral = detect([
      smallGreen(99),
      smallGreen(99.7),
      smallGreen(100.4),
      smallGreen(101.1),
      doji,
    ]).find((p) => p.kind.includes('doji'));
...
describe('detectCandlePatterns new pattern kinds', () => {
  ...
  it('detects a bullish marubozu with an open-at-low, close-at-high body', () => {
    ...
    expect(
      detect([smallGreen(100), smallRed(100.2), smallGreen(100.1), smallRed(100.2), marubozu]).map(
        (p) => p.kind,
      ),
    ).toContain('bullish_marubozu');
  });
...
describe('detectCandlePatterns real-data regression', () => {
  ...
  it('fires at least one single-bar pattern across the MU intraday fixtures', () => {
    ...
    const allKinds = intradayFixtures.flatMap((name) =>
      detectFromFixture(name).patterns.map((p) => p.kind),
    );
