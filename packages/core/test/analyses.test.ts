...
describe('analyses pub/sub', () => {
  it('delivers a published analysis-created event to the matching symbol', () => {
    ...
    publishAnalysisCreated({
      symbol: 'NVDA.US',
      chartId: '2026-07-06-nvda-intraday',
      type: 'intraday',
    });
  ...
  it('no-ops when publishing to a symbol with no subscribers', () => {
    expect(() =>
      publishAnalysisCreated({ symbol: 'NOSUB.US', chartId: 'c1', type: 'intraday' }),
    ).not.toThrow();
  });
  ...
  it('isolates a throwing subscriber from later subscribers', () => {
    ...
    const unsubGood = subscribeAnalyses('BUS.US', (envelope) =>
      received.push(JSON.parse(envelope)),
    );
    expect(() =>
      publishAnalysisCreated({ symbol: 'BUS.US', chartId: 'c1', type: 'intraday' }),
    ).not.toThrow();
