...
import type { CandlePattern } from '@kansoku/shared/types';
import {
...
function weakBullish(
  ctx: PatternScoringContext,
  i: number,
  overrides: Partial<CandlePattern> = {},
): CandlePattern {
  return {
    kind: 'hammer',
    time: ctx.timesTs[i],
...
describe('enrichCandlePatterns', () => {
  ...
  it('walks the confirmation state machine off the next three closes', () => {
    ...
    expect(enrichCandlePatterns([weakBullish(confirmed, 30)], confirmed)[0].status).toBe(
      'confirmed',
    );

    const invalidated = makeCtx(40);
    invalidated.closes[31] = 98.5;
    expect(enrichCandlePatterns([weakBullish(invalidated, 30)], invalidated)[0].status).toBe(
      'invalidated',
    );
