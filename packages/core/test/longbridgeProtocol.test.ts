...
describe('Longbridge realtime protocol', () => {
  ...
  it('decodes response and gzip push packets', () => {
    ...
    const push = decodePacket(
      bytes(
        0x23,
        101,
        (zipped.length >>> 16) & 0xff,
        (zipped.length >>> 8) & 0xff,
        zipped.length & 0xff,
        ...zipped,
      ),
    );
  ...
  it('encodes security quote and candlestick query requests', () => {
    expect([...encodeMultiSecurityRequest(['AAA.US', 'BBB.US'])]).toEqual([
      ...str(1, 'AAA.US'),
      ...str(1, 'BBB.US'),
    ]);
  ...
  it('decodes capital distribution responses with in/out buckets', () => {
    ...
    expect(
      decodeCapitalDistributionResponse(
        bytes(...str(1, 'A.US'), ...num(2, 60), ...msg(3, capitalIn), ...msg(4, capitalOut)),
      ),
    ).toEqual({
      symbol: 'A.US',
