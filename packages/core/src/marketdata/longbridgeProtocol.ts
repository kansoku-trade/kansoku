...
export function encodeAuthRequest(token: string, metadata: Record<string, string>): Uint8Array {
  return concat([
    fieldString(1, token),
    ...Object.entries(metadata).map(([key, value]) => mapStringEntry(2, key, value)),
  ]);
}
...
export function encodeUnsubscribeRequest(
  symbols: string[],
  subTypes: number[],
  all = false,
): Uint8Array {
...
export function encodeCandlestickRequest(
  symbol: string,
  period: number,
  count: number,
...
export function decodeStaticNameResponse(
  body: Uint8Array,
...
function decodeCapitalBucket(body: Uint8Array | null): {
  large: string;
  medium: string;
  small: string;
} {
