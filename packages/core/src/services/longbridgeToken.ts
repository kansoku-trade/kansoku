...
function parseStoredToken(value: StoredToken, dcRegion: string | null): LongbridgeToken {
  ...
  if (expiresAt > 0 && expiresAt <= Math.floor(Date.now() / 1000)) {
    throw new LongbridgeTokenError("Longbridge Token 已过期，请重新执行 longbridge auth login", "TOKEN_EXPIRED");
  }
  return {
...
function decryptTokenFile(data: Buffer, machineId: string): StoredToken {
  if (data.length < MAGIC.length + NONCE_BYTES + TAG_BYTES || !data.subarray(0, MAGIC.length).equals(MAGIC)) {
  ...
  const key = Buffer.from(hkdfSync("sha256", Buffer.from(machineId), Buffer.alloc(0), HKDF_INFO, 32));
  try {
    const decipher = createDecipheriv("aes-256-gcm", key, data.subarray(nonceStart, ciphertextStart));
    decipher.setAuthTag(data.subarray(tagStart));
    const plaintext = Buffer.concat([decipher.update(data.subarray(ciphertextStart, tagStart)), decipher.final()]);
    return JSON.parse(plaintext.toString("utf8")) as StoredToken;
  } catch {
    throw new LongbridgeTokenError("无法解密 Longbridge Token，请重新执行 longbridge auth login", "TOKEN_UNREADABLE");
  }
}

export async function readLongbridgeToken(deps: LongbridgeTokenDeps = {}): Promise<LongbridgeToken> {
  ...
  if (!tokenStatus || ["not_found", "expired", "decrypt_failed"].includes(tokenStatus)) {
    throw new LongbridgeTokenError("Longbridge CLI 尚未登录，请执行 longbridge auth login", "NOT_LOGGED_IN");
  }
