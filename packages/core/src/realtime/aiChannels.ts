...
function attachNotifications(push: (envelope: string) => void): () => void {
  const unsubComment = onAnyComment((comment) =>
    push(JSON.stringify({ type: 'comment', comment })),
  );
...
function attachAnalystRuns(push: (envelope: string) => void): () => void {
  const unsub = onAnalystRunChange((symbol, status) =>
    push(JSON.stringify({ type: 'update', symbol, status })),
  );
...
const assistantChatChannel: ProChannel = {
  ...
  attach: (msg, push) =>
    attachConversation(msg.id as string, push, onAssistantChatEvent, assistantChatTurnState),
};
