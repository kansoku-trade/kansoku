...
2. **部署**：
   ...
   ```
3. **绑定自定义域名**：客户端默认指向 `https://license.kansoku.app`（`packages/core/src/license/dodoClient.ts` 的 `DEFAULT_LICENSE_API_URL`，可用 `KANSOKU_LICENSE_API_URL` 覆盖）。在 Cloudflare Dashboard 的 Worker 设置里给 `kansoku-license-worker`（生产环境）加一条 Custom Domain 指向 `license.kansoku.app`，或者在 `wrangler.jsonc` 的 `env.production` 下加 `routes`/`custom_domain` 配置后重新 `deploy`。domain 生效前，客户端会打到 Worker 默认的 `*.workers.dev` 地址（`deploy` 命令输出里能看到），可以先拿它联调。
