...
export type ConversationChannelSpec =
  | { kind: 'chat'; id: string }
...
interface ConversationAdapter {
  ...
  channel: (id: string) => ConversationChannelSpec;
...
export interface ChatTransportClient {
  chat: {
    ...
    postMessage(input: { id: string; text: string }): Promise<{ status: number; body: unknown }>;
    abort(input: { id: string }): Promise<unknown>;
    suggestions(input: { id: string }): Promise<{ suggestions: string[] }>;
  };
  research: {
    getChat(input: { path: string }): Promise<unknown>;
    postMessage(input: { path: string; text: string }): Promise<{ status: number; body: unknown }>;
    abortChat(input: { path: string }): Promise<unknown>;
    suggestions(input: { path: string }): Promise<{ suggestions: string[] }>;
  };
  assistant: {
    getChat(input: { id: string }): Promise<unknown>;
    postMessage(input: { id: string; text: string }): Promise<{ status: number; body: unknown }>;
    abortChat(input: { id: string }): Promise<unknown>;
  };
}

export type ChatSubscribe = (
  spec: ConversationChannelSpec,
...
export interface ResearchChatTransport {
  client: ChatTransportClient['research'];
  subscribe: ChatSubscribe;
}

export function createConversationAdapters(
  client: ChatTransportClient,
): Record<ConversationKind, ConversationAdapter> {
  return {
    chart: {
    ...
    assistant: {
      fetchChat: async (id) =>
        (await client.assistant.getChat({ id })) as unknown as ChatEnvelope,
...
export function useConversationSession(
  ...
  const { client: transportClient, subscribe } = transport;
  const adapter = useMemo(
    () => createConversationAdapters(transportClient)[kind],
    [transportClient, kind],
  );
  ...
  useEffect(() => {
    ...
    return off;
  }, [adapter, id, enabled, subscribe, reload, streamFlush, streamFinish, streamPush, streamReset]);
...
export function useResearchConversationSession(
  ...
  const fullTransport: ChatTransport = useMemo(
    () => ({
      subscribe: transport.subscribe,
      client: { research: transport.client } as ChatTransportClient,
    }),
    [transport.client, transport.subscribe],
  );
  return useConversationSession('research', path, enabled, fullTransport);
}
