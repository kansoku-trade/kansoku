    ...
    <div className="research-refresh-progress" aria-label="研究任务进度">
      <div className="research-refresh-steps">
        {PHASES.map((item, index) => {
          ...
          return (
            <div
              className={`research-refresh-step research-refresh-step--${state}`}
              key={item.phase}
            >
      ...
      {findings.map((finding, index) => (
        <article key={`${finding.title}:${index}`}>
          <header>
            ...
            <span
              className={`research-refresh-confidence research-refresh-confidence--${finding.confidence}`}
            >
          ...
          <div className="research-refresh-citations" aria-label="引用证据">
            {finding.evidenceIds.map((id) => (
              <code key={id} title={evidence.get(id)?.title}>
                {id}
              </code>
            ))}
          </div>
...
function ResearchReport({
  ...
  return (
    <div className="research-refresh-report">
      ...
      {report.proposalId ? (
        <div className="research-refresh-proposal">
