...
describe("research list presentation", () => {
  it("removes a duplicated date while preserving the meaningful title", () => {
    expect(researchListTitle(document({ title: "2026-07-14 09:20（美东，重估）", date: "2026-07-14" }))).toBe(
      "09:20（美东，重估）",
    );
    expect(researchListTitle(document({ title: "MU 短线多周期分析 — 2026-07-06", date: "2026-07-06" }))).toBe(
      "MU 短线多周期分析",
    );
  });
  ...
  it("compacts ISO timestamps without changing the stored title", () => {
    expect(
      researchListTitle(document({ title: "2026-07-09T08:53:28Z DRAM.US intraday-signal", date: "2026-07-09" })),
...
describe("research relationships", () => {
  it("links a journal to matching stock notes and other records without filename assumptions", () => {
    ...
    const unrelated = document({ path: "stocks/MSFT.md", kind: "stock", type: "stock", symbols: ["MSFT"] });
