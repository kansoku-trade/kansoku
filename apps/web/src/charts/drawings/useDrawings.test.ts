...
describe('decodeAnnotationsFrame', () => {
  ...
  it('accepts an update frame from another client', () => {
    expect(
      decodeAnnotationsFrame({ type: 'update', annotations: sample, clientId: 'other' }, 'me'),
    ).toBe(sample);
  });
  ...
  it('ignores an update frame echoing its own clientId', () => {
    expect(
      decodeAnnotationsFrame({ type: 'update', annotations: sample, clientId: 'me' }, 'me'),
    ).toBeNull();
  });
...
describe('mergePendingRemote', () => {
  const remoteOnly: Annotation = {
    id: 'r1',
    kind: 'hline',
    points: [{ time: 1, price: 2 }],
    createdAt: 1,
  };
  const localNew: Annotation = {
    id: 'l1',
    kind: 'hline',
    points: [{ time: 3, price: 4 }],
    createdAt: 2,
  };
  const sharedRemote: Annotation = {
    id: 's1',
    kind: 'hline',
    points: [{ time: 5, price: 6 }],
    createdAt: 3,
  };
  const sharedLocalStale: Annotation = {
    id: 's1',
    kind: 'hline',
    points: [{ time: 5, price: 99 }],
    createdAt: 3,
  };
