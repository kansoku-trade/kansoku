...
**插槽怎么接**：`apps/pro/` 是私有仓库 `kansoku-pro` 的源码所在（本地开发时以 git worktree 形式链接进来，见 `kansoku-workspace` 超工程）。官方构建时它被打包、加密成一份 `pro.enc`（AES-256-GCM，见 `apps/pro/scripts/packEnc.mjs`），随发行版分发——源码不会以任何形式进入这个公开仓库。server（`runtimeInit.ts`）与 desktop（`boot/kernel.ts`，实际也是复用 `runtimeInit.ts`）启动早期都会调 `packages/core/src/pro/editionLoader.ts` 的 `loadEdition()`：定位 `pro.enc`、按 `KANSOKU_BUNDLE_KEY` 解密、校验 ABI 版本（`EDITION_ABI_VERSION`）与打包时绑定的公开仓库 commit，通过 `encLoader.ts` 的虚拟模块 hook 动态 `import()` 出一个 `EditionEntry`。`pro.enc` 不存在、密钥不对、ABI 或 commit 不匹配，都会被判成非 `active` 的激活状态（`absent`/`locked`/`incompatible`/`failed`），落进免费的 `ServerEdition`/`DesktopEdition`——一行 info/error 日志，不报错不刷屏。公开代码永远不会静态 import 私有源码，社区 clone 下来正常 `pnpm install`/`typecheck`/`build`/`test`/`dev` 全部照常跑通，唯一区别是 AI 入口在 UI 上不出现。

**接口约定**：`packages/pro-api`（`@kansoku/pro-api`，公开、纯类型包）定义了 edition 协议的 ABI 形状——`EditionEntry`（`abiVersion`/`runtime`/`createEdition`）、`EditionProCapabilities`（`hooks`/`aiExtension`）等。解密出来的 edition 通过 `configureServer`/`configureIpc`/`configureRealtime`/`proCapabilities()` 几个方法把自己接进宿主（见 `packages/core/src/edition/base.ts`）；`proCapabilities().hooks` 是 `EditionHooks`（非 AI 代码需要反查的东西，比如跟进状态、深度研究），由 `packages/core/src/pro/domain/defaultImplementations.ts` 桥接成 core 认识的 `FollowAutomation`/`DeepDiveService`/`AiTurnPipeline`。免费模式下这些接口都有空实现（`DisabledFollowAutomation` 等）——所以就算 `pro.enc` 缺失，调用这些能力的代码也不用到处判空。`packages/core/src/pro/editionRuntime.ts` 的 `EditionRuntime` 把当前 edition 的激活状态广播给 capabilities 服务等所有读者。
