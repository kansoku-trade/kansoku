...
describe('dataImport', () => {
  ...
  describe('validateImportSource', () => {
    ...
    it('rejects a source missing journal/charts/data entirely', () => {
      ...
      expect(validateImportSource(bareRoot, dest)).toEqual({
        ok: false,
  ...
  describe('buildImportManifest', () => {
    ...
    it('flags files that already exist at the destination as collisions', () => {
      ...
      const collided = manifest.entries.find((entry) =>
        entry.relPath.endsWith('NVDA-2026-01-01.json'),
      );
      expect(collided?.exists).toBe(true);
      const clean = manifest.entries.find((entry) =>
        entry.relPath.endsWith('MRVL-2026-01-02.json'),
      );
  ...
  describe('copyImportManifest', () => {
    it('copies every entry when there are no collisions', () => {
      ...
      expect(
        readFileSync(join(dest, 'journal', 'charts', 'data', 'NVDA-2026-01-01.json'), 'utf8'),
      ).toBe('{source}');
    });
    ...
    it('creates the destination chart data dir if it does not already exist', () => {
      ...
      expect(existsSync(join(freshDest, 'journal', 'charts', 'data', 'NVDA-2026-01-01.json'))).toBe(
        true,
      );
    });
    ...
    it('reports a per-file failure without aborting the rest of the batch', () => {
      ...
      expect(result.failures[0].relPath).toBe(
        join('journal', 'charts', 'data', 'NVDA-2026-01-01.json'),
      );
      expect(result.failures[0].error).toBeTruthy();
      expect(
        readFileSync(join(dest, 'journal', 'charts', 'data', 'MRVL-2026-01-02.json'), 'utf8'),
      ).toBe('{mrvl}');
      expect(existsSync(join(dest, 'journal', 'charts', 'data', 'NVDA-2026-01-01.json'))).toBe(
        false,
      );
    });
  });
