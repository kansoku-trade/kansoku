...
import { SecretBoxError } from "../../../../packages/core/src/services/secretCrypto.js";
...
describe("desktopSecretBox", () => {
  ...
  it("legacy path: the wrapped key matches the legacy key material (decryptable across box instances)", () => {
    ...
    const boxA = createDesktopSecretBox({ safeStorage: fakeSafeStorage(), wrappedKeyPath, legacyKeyPath });
    const envelope = boxA.encrypt("longbridge", "secret-value");

    rmSync(legacyKeyPath);
    const boxB = createDesktopSecretBox({ safeStorage: fakeSafeStorage(), wrappedKeyPath, legacyKeyPath });
