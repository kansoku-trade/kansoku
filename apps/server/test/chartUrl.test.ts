...
describe("chartTargetPath", () => {
  it("routes intraday charts to the symbol page pinned to the analysis", () => {
    expect(chartTargetPath(doc({ type: "intraday" }))).toBe("/symbol/MRVL.US?analysis=2026-07-02-mrvl-intraday");
  });
  ...
  it("routes flow charts to the home page for that market date", () => {
    expect(chartTargetPath(doc({ type: "flow", symbol: null, id: "2026-07-02-flow" }))).toBe("/?date=2026-07-02");
  });

  it("routes cohort charts to the home page for that market date", () => {
    expect(chartTargetPath(doc({ type: "cohort", symbol: null, id: "2026-07-02-cohort" }))).toBe("/?date=2026-07-02");
  });
  ...
  it("encodes special characters in symbol and id", () => {
    expect(symbolAnalysisPath("MRVL US", "id with space")).toBe("/symbol/MRVL%20US?analysis=id%20with%20space");
  });
