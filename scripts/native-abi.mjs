...
const coreRequire = createRequire(join(rootDir, 'packages', 'core', 'package.json'));

const CACHE_DIR = join(rootDir, 'node_modules', '.abi-cache');
...
function cachePath(root, abiTag) {
  return join(
    CACHE_DIR,
    `better-sqlite3@${pkgVersion(root)}-${platform()}-${process.arch}-${abiTag}.node`,
  );
}
...
export function assertNoElectronDev(label) {
  ...
  const running = String(ps.stdout)
    .split('\n')
    .some(
      (line) => line.includes('Electron.app/Contents/MacOS/Electron') && line.includes(rootDir),
    );
  if (running) {
    console.error(
...
function ensure({ label, abiTag, probe, slowRebuild }) {
  ...
  if (result.status !== 0) {
    console.error(
      `[${label}] better-sqlite3 still unusable after rebuild (status=${result.status ?? result.signal})`,
    );
...
export function ensureElectronAbi(desktopRoot, label) {
  ...
  const electronVersion = desktopRequire('electron/package.json').version;
    ...
    slowRebuild: () => {
      const result = spawnSync('pnpm', ['run', 'rebuild-native'], {
        cwd: desktopRoot,
        stdio: 'inherit',
      });
